...
setup_omz() {
    ...
    if has_command curl; then
        if ! run_cmd sh -c "$(curl -fsSL https://raw.githubusercontent.com/ohmyzsh/ohmyzsh/master/tools/install.sh)" "" --unattended; then
            error "Failed to install Oh My Zsh via curl"
            return 1
        fi
    elif has_command wget; then
        if ! run_cmd sh -c "$(wget -O- https://raw.githubusercontent.com/ohmyzsh/ohmyzsh/master/tools/install.sh)" "" --unattended; then
            error "Failed to install Oh My Zsh via wget"
            return 1
        fi
    else
        error "Neither curl nor wget is available to download Oh My Zsh"
        return 1
    fi
    
    # Verify installation
    if [[ ! -d "${HOME}/.oh-my-zsh" ]]; then
        error "Oh My Zsh installation verification failed"
        return 1
    fi
...
configure_zsh() {
    ...
    local zsh_config_target="${HOME}/.zshrc"
    
    # Determine which zsh config to use
    if [[ $(uname -m) == "x86_64" ]]; then
        zsh_config_source="${dotfiles_dir}/configs_pc/zsh"
    else
    ...
    if [[ -d "${zsh_config_source}" ]]; then
        info "Linking ZSH configuration from ${zsh_config_source}"
        
        # Backup existing config
        if [[ -f "${zsh_config_target}" && ! -L "${zsh_config_target}" ]]; then
            local backup="${zsh_config_target}.backup.$(date +%Y%m%d%H%M%S)"
            info "Backing up existing ZSH config to ${backup}"
            if ! run_cmd mv "${zsh_config_target}" "${backup}"; then
                error "Failed to backup existing ZSH config"
                return 1
            fi
        fi
        
        # Create symbolic link
        if ! create_symlink "${zsh_config_source}/.zshrc" "${zsh_config_target}"; then
            error "Failed to link ZSH configuration"
            return 1
        fi
        ...
        for config_file in "${zsh_config_source}"/*.zsh; do
            if [[ -f "${config_file}" ]]; then
                ...
                if ! create_symlink "${config_file}" "${target}"; then
                    warning "Failed to link ZSH config: ${config_file}"
                fi
            fi
        done
    else
        warning "ZSH configuration directory not found: ${zsh_config_source}"
        warning "Using default Oh My Zsh configuration"
    fi
...
setup_shell() {
    ...
    # Setup Oh My Zsh
    if ! setup_omz; then
        error "Failed to set up Oh My Zsh"
        ((errors++))
    fi
