...
setup_aur_helper() {
	...
	local aur_helpers=("paru" "yay" "pikaur" "trizen" "aurman")
	...
	# Install paru as the default AUR helper
	info "Installing paru AUR helper"
	
	# Check if rustup is installed or install it
	if ! has_command rustup; then
		info "Setting up Rust for AUR helper"
		if ! bash "${SCRIPT_DIR}/scripts/setup_rust.sh"; then
	...
	# Check again for cargo
	if ! has_command cargo; then
		error "Cannot find cargo for building AUR helper"
		return 1
	fi
	
	# Install paru using cargo
	if ! run_cmd cargo install paru; then
	...
	if ! has_command paru; then
		error "paru installation verification failed"
		
		# Try adding cargo bin to PATH
		if [[ ! ":${PATH}:" == *":${HOME}/.cargo/bin:"* ]]; then
			export PATH="${HOME}/.cargo/bin:${PATH}"
			
			if ! has_command paru; then
				error "paru not found even after updating PATH"
				return 1
			fi
		else
			return 1
