...
clone_repo_with_progress() {
  local repo_url="$1"
  local dest_dir="${2:-$(basename "$repo_url" .git)}"
  local progress_file="/tmp/git_clone_progress_$$"
  local clone_pid
  local clone_status
  ...
  # Start git clone in background with progress
  git clone --progress "$repo_url" "$dest_dir" 2>"$progress_file" &
  clone_pid=$!
  ...
  while kill -0 "$clone_pid" 2>/dev/null; do
    if [[ -f "$progress_file" ]]; then
      # Extract latest percentage
      local percentage
      percentage=$(grep -oE '[0-9]+%' "$progress_file" | tail -1)
      [[ -n "$percentage" ]] && echo -ne "Progress: $percentage\r"
    fi
    sleep 0.5
  done

  # Wait for clone to complete and check status
  wait "$clone_pid"
  clone_status=$?
  ...
  echo -e "\nClone completed successfully to $dest_dir"
  return 0
}
...
alias claude="/home/utsav/.config/claude/local/claude"
